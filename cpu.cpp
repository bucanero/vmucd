...
int t1h, t1l;
/*
void writeflashtofile()
{
	file_t f;
	f = fs_open("/pc/modified.vmd", O_WRONLY | O_TRUNC);
	fs_write(f, flash, sizeof(flash));
	fs_close(f);
}
*/
void keypress(int i)
...
int initflash(file_t fd)
{
  int r=0, t=0;
  memset(flash, 0, sizeof(flash));
  printf("Initting Flash\n");
  while(t<sizeof(flash) && (r=fs_read(fd, flash+t, sizeof(flash)-t))>0)
    t += r;
  if(r<0 || t<0x480)
    return 0;
  return t;
...
int loadflash(char *filename)
{
  ...
  if((fd = fs_open(filename, O_RDONLY|O_BINARY))) {
    if(!(r=initflash(fd))) {
      fs_close(fd);
      printf("%s: can't load flash image\n", filename);
    ...
    printf("%s: error loading flash image\n", filename);
    return 0;
  }
}

int initbios(file_t fd)
{
  int r=0, t=0;
  memset(bios, 0, sizeof(bios));
  printf("Initing BIOS\n");
  while(t<sizeof(bios) && (r=fs_read(fd, bios+t, sizeof(bios)-t))>0)
    t += r;
  if(r<0 || t<0x200)
    return 0;
  return 1;
...
void halt_mode()
{
  /* waitforevents(NULL); Commented out due to useless function call */
  checkevents();
}
...
void run_cpu()
{
  ...
  for(;;) {
	...
	epoch.tv_usec -= 1000000;   // adapted to lengthen cycle time since
				   // dc doesn't have usec timer resolution
	epoch.tv_sec++;
      }
	...
	t =  0;
        ...
        t = (epoch.tv_sec - now.tv_sec - 1) * 1000 + 
            (1000000 + epoch.tv_usec - now.tv_usec) / 1000;
/*	t.tv_usec = 1000000 + epoch.tv_usec - now.tv_usec;
	t.tv_sec = epoch.tv_sec - now.tv_sec - 1;*/
      } else {
        t = (epoch.tv_usec - now.tv_usec) / 1000 + 
            (epoch.tv_sec - now.tv_sec) * 1000;
